...
/**
 * Layout: Login Modal
 * Figma: — (sem nodeId ainda; derivado de FEATURE-cadastro.md §6.1)
 *
 * Modal de autenticação exibido sobre qualquer página do portal. Em desktop
 * `≥ md` aparece como card centralizado (max-w-[480px], rounded-lg, shadow-lg)
 * sobre overlay bg-primary-950/[.32]. Em mobile `< md` vira bottom-sheet
 * (rounded-t-2xl) — quando for integrado ao runtime real, o wrapper externo
 * troca para o partial `bottom-sheet` nesse breakpoint.
 *
 * Este layout é um showcase spec dos 7 estados obrigatórios definidos em
 * §6.1. O dev que integrar no tema real usa a marcação de qualquer um dos
 * 7 blocos conforme o contexto.
 *
 * Componentes consumidos (partials):
 *   - header-desktop, footer-desktop, icon-button, divider, form-field,
 *     link-button
 *
 * Interatividade:
 *   - Todos os elementos interativos têm data-attrs para o auth-modal.js:
 *     data-login-action="close" | "toggle-password" | "resend-confirmation"
 *   - Inputs de senha já vêm com botão olho (toggle visibility).
 *   - Countdown de bloqueio renderizado com aria-live="polite".
 *
 * Tagueamento Analytics (§8):
 *   - data-analytics-event="login_iniciado" — abertura do modal
 *   - data-analytics-event="login_erro"     — submit com falha
 *   - data-analytics-event="login_concluido"— submit bem-sucedido
 */

/**
 * Renderiza o modal em um dos 7 estados.
 *
 * @param string $state  default | loading | error-credentials | error-unverified
 *                       | locked | session-expired | post-download
 */
...
?>
  ...
  >
    ...
    <div class="mt-6 flex flex-col gap-3">
      <button type="button" <?= $state === 'default' ? "onclick=\"window.location='/src/layouts/onboarding.php'\"" : '' ?> class="inline-flex items-center justify-center gap-3 w-full h-12 rounded-lg border-[1.5px] border-neutral-200 bg-white hover:bg-neutral-50 transition-colors font-body font-semibold text-body-lg text-neutral-950 disabled:opacity-50" <?= $formDisabled ? 'disabled' : '' ?>>
      ...
      <button type="button" <?= $state === 'default' ? "onclick=\"window.location='/src/layouts/onboarding.php'\"" : '' ?> class="inline-flex items-center justify-center gap-3 w-full h-12 rounded-lg border border-neutral-200 bg-white hover:bg-neutral-50 transition-colors font-body font-semibold text-body-lg text-neutral-950 disabled:opacity-50" <?= $formDisabled ? 'disabled' : '' ?>>
    ...
    <!-- Form (action="/" só no default — submit leva para home simulando login ok) -->
    <form class="mt-6 flex flex-col gap-4" <?= $state === 'default' ? 'action="/" method="get"' : '' ?> <?= $formDisabled ? 'aria-disabled="true"' : '' ?> data-analytics-event="login_iniciado">
      <fieldset class="contents" <?= $formDisabled ? 'disabled' : '' ?>>
        ...
        <div class="flex justify-end">
          <?php get_template_part('components/_partials/link-button', null, [
            ...
            'href'  => '/src/layouts/recupera-senha.php',
            'size'  => 'sm',
          ]); ?>
        </div>
    ...
    <p class="mt-6 text-center font-body text-body-md text-neutral-700">
      Não tem conta?
      <a href="/src/layouts/cadastro-bloco-1.php" class="font-bold text-secondary-950 hover:underline">Criar conta</a>
    </p>
  </div>
<?php }

$states = [
  'default'           => ['title' => 'Default',                  'desc' => 'Form limpo, pronto para uso.'],
  'loading'           => ['title' => 'Loading',                  'desc' => 'Após submit — spinner inline + form bloqueado.'],
  'error-credentials' => ['title' => 'Erro: credenciais',        'desc' => 'Alert error genérico — NUNCA indica qual campo falhou.'],
  'error-unverified'  => ['title' => 'Erro: conta não verificada','desc' => 'Warning + ação de reenviar e-mail de confirmação.'],
  'locked'            => ['title' => 'Bloqueio 15 minutos',      'desc' => 'Após 5 tentativas — form desabilitado, countdown em tempo real.'],
  'session-expired'   => ['title' => 'Sessão expirada',          'desc' => 'Subtítulo diferenciado + warning.'],
  'post-download'     => ['title' => 'Redirect pós-download',    'desc' => 'Badge de contexto acima do título.'],
];
?>
<main class="bg-white min-h-screen">
  ...
  <!-- ========================================================================
       Preview principal: modal default sobre página dimmed
       ======================================================================== -->
  <section class="relative">
    ...
    <div class="absolute inset-0 bg-primary-950/[.32] flex items-center justify-center p-4" aria-hidden="false">
      <?php render_login_modal('default'); ?>
    </div>
  </section>

  <!-- ========================================================================
       Galeria dos 7 estados — referência para devs e QA
  ...
  <section class="bg-white py-16">
    <div class="max-w-screen-xl mx-auto px-4 lg:px-6">
      <header class="mb-10 max-w-3xl">
        ...
        <h2 class="mt-2 font-display text-headline-md text-neutral-950">7 estados obrigatórios</h2>
        <p class="mt-3 font-body text-body-lg text-neutral-700">
          Cada card abaixo mostra a marcação final do modal em um dos estados definidos em
          <code class="font-mono text-label-md bg-neutral-50 rounded-xs px-1 py-0.5">FEATURE-cadastro.md §6.1</code>.
          Copiar o bloco completo para integração.
        </p>
      </header>
      ...
      <div class="space-y-16">
        <?php foreach ($states as $key => $meta): ?>
          <article>
            ...
            <div class="bg-primary-950/[.32] rounded-lg p-6 md:p-10 flex justify-center">
              <?php render_login_modal($key); ?>
            </div>
          </article>
        <?php endforeach; ?>
      </div>
