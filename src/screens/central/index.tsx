...
const GROUPS: RouteGroup[] = [
	...
	},
	{
		...
		description: 'Modais que convertem visitante deslogado em cadastro — em sua maioria reusando os mesmos modais de Cadastro/Login acima.',
		color: 'secondary',
		items: [
	...
	},
	{
		...
		color: 'primary',
		items: [
			{ path: '/conteudo?post=post-autor-unico', label: 'Autor único', description: '1 autor · 2 headings, sem TOC', status: 'pronto-para-dev' },
			{ path: '/conteudo?post=post-download', label: 'Download', description: 'Bloco de download no corpo do artigo', status: 'pronto-para-dev' },
			{ path: '/conteudo?post=post-longo-toc', label: 'Table of Contents (TOC)', description: '13 headings · régua na margem esquerda a partir de 1536px, botão flutuante "Neste artigo" abaixo disso', status: 'em-aprovacao' },
		],
	},
	{
	...
	},
	{
		...
		items: [
			...
			{ path: '/email-exclusao-conta', label: 'Exclusão de conta', description: 'Confirmação de exclusão imediata', status: 'pronto-para-dev' },
		],
	},
	{
		title: 'Sistema',
...
const STATUS_META: Record<RouteStatus, { label: string; tone: BadgeTone }> = {
	...
	'em-aprovacao': { label: 'Em aprovação', tone: 'saffron' },
			...
			<div className="max-w-screen-xl mx-auto px-4 lg:px-6 py-16">
				<header className="flex flex-col gap-4 pb-12 border-b border-neutral-100">
					...
					<p className="font-body text-body-sm text-neutral-700 flex flex-wrap items-center gap-x-2 gap-y-1.5">
						...
						<Badge label="Em aprovação" tone="saffron" /> com o cliente para validar ·{' '}
