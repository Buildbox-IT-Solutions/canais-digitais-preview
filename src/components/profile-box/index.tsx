...
export function ProfileBox({
	...
	const columnPadding = incomplete ? 'pt-3 pb-6 px-6' : 'p-6'
	...
	return (
		<div className={twMerge('bg-white border border-neutral-100 rounded-lg w-full', className)}>
			...
			<div className="flex items-stretch w-full">
				<div className={twMerge('flex-1 min-w-0 flex items-start gap-4', columnPadding)}>
				...
				<div className={twMerge('flex-1 min-w-0 flex flex-col gap-1', columnPadding)}>
					{fields.map((f, i) => (
						<p key={i} className={twMerge('font-body text-body-md truncate', fieldTextColor)}>
							{f}
						</p>
					))}
				</div>
				...
				<div
					className={twMerge('shrink-0 flex flex-col items-end justify-center', columnPadding)}
				>
					<a
						href={href}
