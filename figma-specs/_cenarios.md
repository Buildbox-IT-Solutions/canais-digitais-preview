...
**Status legend:** ⬜ a fazer · 🔄 em progresso · ✅ feito
...
|---|---|---|---|---|
| **Novo** (recém-criado) | ~0% / placeholder | vazio | vazio | nenhuma ativa (0/6) |
| **Parcial** (default de hoje) | ~40% | 24 itens | populada | 2 de 6 |
| **Engajado / completo** | **100%** | populada | populada | todas (6/6) |

**Proposta de orquestração:** um `?persona=novo|engajado` (default = parcial) que atravessa as 4 abas de uma vez, mantendo o `?state=saved` (toast) ortogonal. Decisão de param a confirmar na hora de construir.
...
- **Onde:** aba Downloads do `dashboard-perfil-v4` (`?tab=downloads`).
- **Gatilho proposto:** `?persona=novo` (ou `?state=empty` estendido para a aba).
- **O que mostrar:** bloco vazio "Você ainda não baixou nenhum material" + subtexto + CTA "Explorar conteúdos" → `/home` (ou `/categoria`).
- **Reaproveitar:** o padrão de empty já usado no `UltimasPane` ([dashboard-perfil-v4:170-188](../src/screens/dashboard-perfil-v4/index.tsx)) — card `bg-neutral-50` + ícone em círculo + título + texto + botão. Ícone `download`.

### 2. Perfil — 100% completo ⬜
- **Onde:** aba Perfil (`?tab=perfil`).
- **Gatilho proposto:** `?persona=engajado`.
- **O que muda:** anel de progresso em **100%**; remover o banner "Faltam N campos" e os chips "Complete seu Perfil"/"incomplete" dos `ProfileBox`; mensagem de celebração (ex.: "Perfil completo ✓"). `ProfileBox` sem estados `incomplete`/`placeholder`.
- **Reaproveitar:** `ProfileMetrics`, `ProfileBox`; mock `PERFIL_CAMPOS` totalmente preenchido → `pct` = 100 automaticamente.

### 3. Perfil — 0% / recém-criado ⬜
- **Onde:** aba Perfil.
- **Gatilho proposto:** `?persona=novo`.
- **O que muda:** anel em **0%**; todos os campos em placeholder; banner "complete seu perfil" reforçado. Hoje o mock é parcial — falta o extremo "acabou de criar".
- **Reaproveitar:** mesmos componentes; `PERFIL_CAMPOS` vazio.
